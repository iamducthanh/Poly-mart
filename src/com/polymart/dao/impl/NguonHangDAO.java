...
public class NguonHangDAO extends AbstractDAO<NguonHangModel> implements INguonHangDAO {
	...
	public Integer save(NguonHangModel nguonHangModel) {
		...
		sql.append(" VALUES (?,?,?)");
		return insert(sql.toString(), nguonHangModel.getTenNguonHang(), nguonHangModel.getSdt(), nguonHangModel.getDiaChi());
	}
    @Override
	public void update(NguonHangModel nguonHangModel) {
		...
		sql.append(" SODIENTHOAI = ?, DIACHI = ?");
		sql.append(" WHERE ID = ?");
		update(sql.toString(), nguonHangModel.getTenNguonHang(), nguonHangModel.getSdt(), nguonHangModel.getDiaChi(), nguonHangModel.getId());
	}
