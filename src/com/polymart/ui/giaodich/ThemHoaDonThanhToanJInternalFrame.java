			...
			if (row > -1 && row < tbSanPham.getRowCount()) {
				...
				try {
					if (Integer.parseInt(getSoLuong) <= chiTietSanPhamModel.getSoLuong()) {
						...
						double tongTien = lstChiTietHoaDonThanhToanModels.stream().mapToDouble(
								e -> (chiTietSanPhamModel.getGiaBan().doubleValue() * Integer.parseInt(getSoLuong))
										- (chiTietSanPhamModel.getGiaGiam().doubleValue() * e.getSoLuong()))
								.sum();
