...
AS
    ...
    SELECT SUM((GIABAN - GIAVON - GIAGIAM - GIAMGIATHEM - DIEMDADOI) * CHITIETHOADONTHANHTOAN.SOLUONG) AS DOANHTHU, 
    SUM(CHITIETHOADONTHANHTOAN.SOLUONG) AS SOLUONGBAN
    FROM CHITIETHOADONTHANHTOAN JOIN CHITIETSANPHAM 
    ON CHITIETHOADONTHANHTOAN.IDCHITIETSANPHAM = CHITIETSANPHAM.ID JOIN HOADONTHANHTOAN
...
AS
	...
	WHERE YEAR(NGAYTHANHTOAN) = 2021 AND MONTH(NGAYTHANHTOAN) = 4
	GROUP BY IDCHITIETSANPHAM

	SELECT IDSANPHAM, TEN, SIZE, MAUSAC, SOLUONGBAN FROM @THONGKEDOANHSO AS THONGKEDOANHSO JOIN CHITIETSANPHAM 
	ON CHITIETSANPHAM.ID = THONGKEDOANHSO.IDCHITIETSP JOIN SANPHAM
